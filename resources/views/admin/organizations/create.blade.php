                    ...
                    <form method="POST" action="{{ route('admin.organizations.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="type" id="type" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="center" {{ old('type') === 'center' ? 'selected' : '' }}>Центр (несколько специалистов)</option>
                                <option value="individual" {{ old('type') === 'individual' ? 'selected' : '' }}>Частный специалист</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
