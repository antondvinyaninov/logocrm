    ...
    <div class="py-12">
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('session-reports.store', $session) }}" class="p-6">
                    @csrf
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="comment" name="comment" rows="6" required
                            ...
                            placeholder="Опишите ход занятия, достижения ребёнка, рекомендации...">{{ old('comment') }}</textarea>
                        @error('comment')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            @for($i = 1; $i <= 5; $i++)
                                <label class="cursor-pointer">
                                    <input type="radio" name="rating" value="{{ $i }}" 
                                        class="peer sr-only" {{ old('rating') == $i ? 'checked' : '' }} required>
