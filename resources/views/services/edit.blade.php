                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="session_type" id="session_type" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="individual" {{ old('session_type', $service->session_type) === 'individual' ? 'selected' : '' }}>Индивидуальное</option>
                            <option value="group" {{ old('session_type', $service->session_type) === 'group' ? 'selected' : '' }}>Групповое</option>
                        </select>
                        @error('session_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="format" id="format" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="both" {{ old('format', $service->format) === 'both' ? 'selected' : '' }}>Онлайн/Офлайн</option>
                            <option value="online" {{ old('format', $service->format) === 'online' ? 'selected' : '' }}>Только онлайн</option>
                            <option value="offline" {{ old('format', $service->format) === 'offline' ? 'selected' : '' }}>Только офлайн</option>
                        </select>
                        @error('format')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="max_participants" id="max_participants" min="1"
                            value="{{ old('max_participants', $service->max_participants) }}"
