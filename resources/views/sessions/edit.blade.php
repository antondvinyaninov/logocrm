<x-home-layout>
    ...
    <div x-data="{ 
        open: true,
        activeTab: 'services',
        selectedService: {{ $session->service_id ? json_encode($services->firstWhere('id', $session->service_id)) : 'null' }},
        selectedChild: {{ json_encode($childrenData->firstWhere('id', $session->child_id)) }},
    ...
    style="display: none;">
        ...
        <div class="flex min-h-screen items-center justify-center p-4">
            <div @click.away="open = false" 
                ...
                <form method="POST" action="{{ route('sessions.update', $session) }}" class="flex h-[85vh]">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="w-64 border-r border-gray-200 bg-white overflow-y-auto">
                        <div class="p-4 space-y-4">
                            <div class="pb-3 border-b border-gray-200">
                                <h3 class="text-sm font-medium text-gray-700">Сотрудник</h3>
                                <p class="text-xs text-gray-500">{{ $session->specialist->full_name }}</p>
                            </div>
                                ...
                                @if(auth()->user()->isOrganization())
                                    <select name="specialist_id" required class="w-full text-sm border-gray-300 rounded-md">
                                        @foreach($specialists as $specialist)
                                            <option value="{{ $specialist->id }}" {{ $session->specialist_id == $specialist->id ? 'selected' : '' }}>
                                                {{ $specialist->full_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                @else
                                    <input type="hidden" name="specialist_id" value="{{ $session->specialist_id }}">
                                    <div class="text-sm font-medium bg-gray-50 p-2 rounded">{{ $session->specialist->full_name }}</div>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="session_date" required
                                    value="{{ old('session_date', $session->start_time->format('Y-m-d')) }}"
                                ...
                                <div class="space-y-2">
                                    <div class="flex gap-2 items-center">
                                        <input type="time" name="session_time" required 
                                            value="{{ old('session_time', $session->start_time->format('H:i')) }}"
                                    ...
                                    <select onchange="document.getElementById('duration_minutes').value = this.value" 
                                        class="w-full text-sm border-gray-300 rounded-md">
                                        <option value="30" {{ $session->duration_minutes == 30 ? 'selected' : '' }}>30 мин</option>
                                        <option value="45" {{ $session->duration_minutes == 45 ? 'selected' : '' }}>45 мин</option>
                                        <option value="60" {{ $session->duration_minutes == 60 ? 'selected' : '' }}>1 ч</option>
                                        <option value="90" {{ $session->duration_minutes == 90 ? 'selected' : '' }}>1 ч 30 мин</option>
                                    </select>
                                    <input type="hidden" name="duration_minutes" id="duration_minutes" value="{{ $session->duration_minutes }}">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="notes" rows="3" 
                                    ...
                                    placeholder="Дополнительная информация...">{{ old('notes', $session->notes) }}</textarea>
                            </div>
                    ...
                    <div class="flex-1 flex flex-col overflow-hidden">
                        ...
                        <div class="px-6 py-4 border-b border-gray-200">
                            <div class="flex items-center gap-3">
                                <label class="flex items-center gap-2 px-4 py-2 bg-gray-100 rounded-lg cursor-pointer hover:bg-gray-200">
                                    <input type="radio" name="status" value="planned" {{ $session->status == 'planned' ? 'checked' : '' }} class="text-indigo-600">
                                    <span class="text-sm font-medium">Ожидание</span>
                                </label>
                                <label class="flex items-center gap-2 px-4 py-2 bg-gray-100 rounded-lg cursor-pointer hover:bg-gray-200">
                                    <input type="radio" name="status" value="confirmed" {{ $session->status == 'confirmed' ? 'checked' : '' }} class="text-green-600">
                                    <span class="text-sm font-medium">Пришел</span>
                                </label>
                                <label class="flex items-center gap-2 px-4 py-2 bg-gray-100 rounded-lg cursor-pointer hover:bg-gray-200">
                                    <input type="radio" name="status" value="cancelled" {{ $session->status == 'cancelled' ? 'checked' : '' }} class="text-red-600">
                                    <span class="text-sm font-medium">Не пришел</span>
                                </label>
                                <label class="flex items-center gap-2 px-4 py-2 bg-gray-100 rounded-lg cursor-pointer hover:bg-gray-200">
                                    <input type="radio" name="status" value="done" {{ $session->status == 'done' ? 'checked' : '' }} class="text-blue-600">
                        ...
                        <div class="flex-1 overflow-y-auto px-6 py-4">
                            ...
                            <div x-show="activeTab === 'services'">
                                ...
                                <input type="hidden" name="service_id" id="service_id" value="{{ $session->service_id }}">
                            ...
                            <div>
                                ...
                                <select name="child_id" @change="selectChild($event.target.value)" required
                                    ...
                                    @foreach($children as $child)
                                        <option value="{{ $child->id }}" {{ $session->child_id == $child->id ? 'selected' : '' }}>
                                            {{ $child->full_name }}
                                        </option>
                                    @endforeach
                                </select>
